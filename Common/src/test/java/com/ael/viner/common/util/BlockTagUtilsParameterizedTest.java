...
class BlockTagUtilsParameterizedTest {
  ...
  void distinguishBlocksAndTags(String input, boolean expectedIsTag) {
    // Act
    boolean actualIsTag = input.startsWith("#") && input.length() > 1 && isValidResourceLocation(input.substring(1));
  ...
  void handleResourceLocationEdgeCases(String input, boolean shouldBeValid, String expectedNamespace, 
                                     String expectedPath, boolean expectedIsTag) {
    // Act
  ...
  void validateNamespacePatterns(String resourceLocation, String expectedNamespace, boolean shouldBeValidNamespace) {
    // Arrange
  ...
  void handleMixedBlockTagLists(String listName, String[] inputs, int expectedBlocks, int expectedTags, int expectedInvalid) {
    // Act
    int actualBlocks = 0;
    int actualTags = 0;
    int actualInvalid = 0;
  ...
  private boolean isValidResourceLocation(String resourceLocation) {
    ...
    return !parts[0].isEmpty() && !parts[1].isEmpty() && isValidNamespace(parts[0]) && isValidPath(parts[1]);
  }
