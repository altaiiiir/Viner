...
/**
 * Enhanced parametrized tests for VineMiningLogic with comprehensive edge cases.
 * Uses parametrization to reduce duplication and test multiple scenarios efficiently.
 */
class VineMiningLogicParameterizedTest {
  ...
  void handleDifferentBlockPatterns(String displayName, Function<Position, String> blockIdAt, 
                                   Position start, String targetBlockId, int expectedSize) {
    // Act
    List<Position> result = VineMiningLogic.collectConnectedBlocks(
        start, targetBlockId, blockIdAt, 1000);
  ...
  void handleDifferentBlockTypes(String blockType) {
    ...
    // Act
    List<Position> result = VineMiningLogic.collectConnectedBlocks(
        start, blockType, mockBlockIdAt, 10);
  ...
  void handleMockFunctionWithComplexBehavior() {
    ...
    // Act
    List<Position> result = VineMiningLogic.collectConnectedBlocks(
        start, targetBlockId, mockBlockIdAt, 10);
  ...
  void handlePerformanceWithLargeAreas() {
    ...
    // Act
    List<Position> result = VineMiningLogic.collectConnectedBlocks(
        start, targetBlockId, blockIdAt, 8000);
  ...
  private static Stream<Arguments> provideBlockPatterns() {
    return Stream.of(
        ...
        Arguments.of(
            "Single Block",
            (Function<Position, String>) pos -> pos.equals(new Position(0, 0, 0)) 
                ? "minecraft:single" : "minecraft:air",
            new Position(0, 0, 0),
            "minecraft:single",
            1
        ),
        ...
        Arguments.of(
            "Straight Line X",
            (Function<Position, String>) pos -> (pos.y() == 0 && pos.z() == 0 && pos.x() >= 0 && pos.x() <= 4) 
                ? "minecraft:line" : "minecraft:air",
            new Position(0, 0, 0),
            "minecraft:line",
            5
        ),
        ...
        Arguments.of(
            "L-Shape",
            (Function<Position, String>) pos -> {
              if (pos.y() == 0 && pos.z() == 0) {
                return (pos.x() >= 0 && pos.x() <= 2) || (pos.x() == 0 && pos.z() >= 0 && pos.z() <= 2) 
                    ? "minecraft:lshape" : "minecraft:air";
              }
        ...
        Arguments.of(
            "Small Cube 2x2x2",
            (Function<Position, String>) pos -> (pos.x() >= 0 && pos.x() <= 1 && 
                                                pos.y() >= 0 && pos.y() <= 1 && 
                                                pos.z() >= 0 && pos.z() <= 1) 
                ? "minecraft:cube" : "minecraft:air",
            new Position(0, 0, 0),
            "minecraft:cube",
            8
        )
