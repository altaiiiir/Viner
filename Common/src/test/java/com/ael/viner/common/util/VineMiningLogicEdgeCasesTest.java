...
/**
 * Edge case and error handling tests for VineMiningLogic. Focuses on boundary conditions, error
 * scenarios, and exceptional cases.
 */
class VineMiningLogicEdgeCasesTest {
  ...
  void handleInvalidLimits() {
    ...
    List<Position> resultZero =
        VineMiningLogic.collectConnectedBlocks(start, targetBlockId, blockIdAt, 0);
    assertEquals(0, resultZero.size(), "Zero limit should return empty list");

    // Act & Assert - Negative limit
    List<Position> resultNegative =
        VineMiningLogic.collectConnectedBlocks(start, targetBlockId, blockIdAt, -5);
  ...
  void handleBlockFunctionReturningNull() {
    ...
    List<Position> result =
        VineMiningLogic.collectConnectedBlocks(start, targetBlockId, blockIdAt, 10);
  ...
  void handleBlockFunctionThrowingExceptions() {
    ...
    when(mockBlockIdAt.apply(any(Position.class)))
        .thenThrow(new RuntimeException("Simulated world access error"));
  ...
  void handleLargeCoordinateValues() {
    // Arrange
    Position start =
        new Position(Integer.MAX_VALUE - 1, Integer.MAX_VALUE - 1, Integer.MAX_VALUE - 1);
  ...
  void handleIntermittentBlockFunctionFailures() {
    ...
    when(mockBlockIdAt.apply(new Position(1, 0, 0)))
  ...
  void handleBlockFunctionWithStateChanges() {
    ...
    when(mockBlockIdAt.apply(any(Position.class)))
        .thenAnswer(
            new org.mockito.stubbing.Answer<String>() {
  ...
  void handleExtremelyHighLimits() {
    ...
    List<Position> result =
        VineMiningLogic.collectConnectedBlocks(start, targetBlockId, blockIdAt, Integer.MAX_VALUE);
  ...
  void handleDisconnectedIdenticalBlocks() {
    ...
    // Assert
    assertEquals(
        1, result.size(), "Should only find connected blocks starting from start position");
    assertEquals(start, result.get(0));
    assertFalse(
        result.contains(new Position(5, 5, 5)), "Should not include disconnected identical blocks");
  }
  ...
  void handleInconsistentBlockFunction() {
    ...
    // Configure mock to return consistent values - algorithm only calls once per position during
    // visited check
    when(mockBlockIdAt.apply(start)).thenReturn(targetBlockId); // Start position matches
    when(mockBlockIdAt.apply(new Position(1, 0, 0)))
        .thenReturn("minecraft:air"); // Adjacent doesn't match
    when(mockBlockIdAt.apply(new Position(-1, 0, 0)))
        .thenReturn("minecraft:air"); // Adjacent doesn't match
    when(mockBlockIdAt.apply(new Position(0, 1, 0)))
        .thenReturn("minecraft:air"); // Adjacent doesn't match
    when(mockBlockIdAt.apply(new Position(0, -1, 0)))
        .thenReturn("minecraft:air"); // Adjacent doesn't match
    when(mockBlockIdAt.apply(new Position(0, 0, 1)))
        .thenReturn("minecraft:air"); // Adjacent doesn't match
    when(mockBlockIdAt.apply(new Position(0, 0, -1)))
        .thenReturn("minecraft:air"); // Adjacent doesn't match
