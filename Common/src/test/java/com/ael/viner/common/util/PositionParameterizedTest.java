...
class PositionParameterizedTest {
  ...
  void testPositionEquality(
      Position pos1, Position pos2, boolean expectedEqual, String description) {
    // Act & Assert
    if (expectedEqual) {
      assertEquals(pos1, pos2, description);
      assertEquals(
          pos1.hashCode(), pos2.hashCode(), "Hash codes should be equal for equal positions");
    } else {
  ...
  void calculateManhattanDistance(
      int x1, int y1, int z1, int x2, int y2, int z2, int expectedDistance) {
    // Arrange
  ...
  void determineAdjacency(
      int x1, int y1, int z1, int x2, int y2, int z2, boolean expectedAdjacent) {
    // Arrange
    Position pos1 = new Position(x1, y1, z1);
    Position pos2 = new Position(x2, y2, z2);
  ...
  void handleCoordinateArithmetic(
      int x1, int y1, int z1, int dx, int dy, int dz, int expectedX, int expectedY, int expectedZ) {
    // Arrange
  ...
  @CsvSource({"0,0,0", "1,2,3", "-1,-2,-3", "1000000,2000000,3000000"})
  ...
  private static Stream<Arguments> providePositionEqualityTestCases() {
    return Stream.of(
        Arguments.of(
            new Position(0, 0, 0), new Position(0, 0, 0), true, "Same coordinates should be equal"),
        Arguments.of(
            new Position(1, 2, 3), new Position(1, 2, 3), true, "Identical positive coordinates"),
        Arguments.of(
            new Position(-1, -2, -3),
            new Position(-1, -2, -3),
            true,
            "Identical negative coordinates"),
        Arguments.of(
            new Position(0, 0, 0), new Position(1, 0, 0), false, "Different X coordinates"),
        Arguments.of(
            new Position(0, 0, 0), new Position(0, 1, 0), false, "Different Y coordinates"),
        Arguments.of(
            new Position(0, 0, 0), new Position(0, 0, 1), false, "Different Z coordinates"),
        Arguments.of(new Position(1, 2, 3), new Position(3, 2, 1), false, "Swapped coordinates"),
        Arguments.of(
            new Position(Integer.MAX_VALUE, 0, 0),
            new Position(Integer.MAX_VALUE, 0, 0),
            true,
            "Max integer values"),
        Arguments.of(
            new Position(Integer.MIN_VALUE, 0, 0),
            new Position(Integer.MIN_VALUE, 0, 0),
            true,
            "Min integer values"));
  }
