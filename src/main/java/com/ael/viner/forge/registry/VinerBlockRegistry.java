...
public class VinerBlockRegistry {
  ...
  private static List<Block> initializeVineableBlocks() {
    List<? extends String> rawList = Config.VINEABLE_BLOCKS.get();
    List<String> stringList = rawList.stream().map(String::valueOf).toList();
    return getBlocksFromConfigEntries(stringList);
  }

  private static List<Block> initializeUnvineableBlocks() {
    List<? extends String> rawList = Config.UNVINEABLE_BLOCKS.get();
    List<String> stringList = rawList.stream().map(String::valueOf).toList();
    return getBlocksFromConfigEntries(stringList);
  }
  ...
  private static List<TagKey<Block>> initializeVineableTags() {
    List<? extends String> rawList = Config.VINEABLE_BLOCKS.get();
    List<String> stringList = rawList.stream().map(String::valueOf).toList();
    return getTagsFromConfigEntries(stringList);
  }
