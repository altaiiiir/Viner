...
public class VinerKeyPressedPacket extends AbstractPacket<Boolean> {
  ...
  public static final PacketFactory<VinerKeyPressedPacket> FACTORY =
      buf -> new VinerKeyPressedPacket(buf.readBoolean());
  ...
  public void handle(AbstractPacket<Boolean> msg, @NotNull Supplier<NetworkEvent.Context> ctx) {
    ...
    context.enqueueWork(
        () ->
            VinerEntrypoint.get()
                .getPlayerRegistry()
                .setVineKeyPressed(context.getSender(), msg.getData()));
  }
}
