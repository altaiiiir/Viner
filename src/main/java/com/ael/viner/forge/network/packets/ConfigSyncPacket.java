...
public class ConfigSyncPacket extends AbstractPacket<ConfigSyncPacket.ConfigData> {
    ...
    switch (data.type()) {
      ...
      case BLOCK_LIST -> {
        ...
        if (value instanceof List<?> rawList) {
          list =
              rawList.stream().allMatch(e -> e instanceof String)
  ...
  public void handle(AbstractPacket<ConfigData> msg, @NotNull Supplier<NetworkEvent.Context> ctx) {
    ...
    if ("vineAll".equals(msg.getData().configName())
      ...
      ((com.ael.viner.forge.registry.VinerPlayerRegistry) VinerEntrypoint.get().getPlayerRegistry())
          .setVineableBlocks(player, entries);
      ((com.ael.viner.forge.registry.VinerPlayerRegistry) VinerEntrypoint.get().getPlayerRegistry())
          .setVineableTags(player, entries);
      ...
      ((com.ael.viner.forge.registry.VinerPlayerRegistry) VinerEntrypoint.get().getPlayerRegistry())
          .setUnvineableBlocks(player, entries);
      ((com.ael.viner.forge.registry.VinerPlayerRegistry) VinerEntrypoint.get().getPlayerRegistry())
          .setUnvineableTags(player, entries);
    }
